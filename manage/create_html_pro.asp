...
sqlpro="select * from product where ver='cn'"
Set rspro= Server.CreateObject("ADODB.Recordset")
rspro.open sqlpro,conn,3,2
do while not rspro.eof
product_id=rspro("product_id")
			if SaveFile("../pro_"&product_id&".html",rooturl&"template/productshow.asp?productid="&product_id&"") then 
...
sql_parent="select * from product_cate where root_id<>0 and ver='cn'"
			...
			do while not rs_small.eof
			   ...
			   PageCount = Int(rs_small.RecordCount / PageSize*-1)*-1
				for k=1 to PageCount
					if SaveFile("../product_"&bigtypes&"_"&smalltypes&"_page"&k&".html",rooturl&"template/product.asp?smalltype="&smalltypes&"&bigtype="&bigtypes&"&page="&k&"") then
...
'生成所有的小类页面
if SaveFile("../product_"&rs_parent("root_id")&"_"&rs_parent("PRO_CATE_ID")&".html",rooturl&"template/product.asp?smalltype="&rs_parent("PRO_CATE_ID")&"&bigtype="&rs_parent("root_id")&"") then 
...
sql_root="select * from product_cate where root_id=0 and ver='cn'"
...
root_id=rs_root("PRO_CATE_ID")
			...
			sql_big="select * from product where ver='cn' and BIGTYPE="&root_id&""
			...
			do while not rs_big.eof
			   ...
			   PageCount = Int(rs_big.RecordCount / PageSize*-1)*-1
					...
					for k=1 to PageCount
						if SaveFile("../product_"&root_id&"_page"&k&".html",rooturl&"template/product.asp?bigtype="&root_id&"&page="&k&"") then
	...
	''生成所有大类的页面		
	if SaveFile("../product_"&root_id&".html",rooturl&"template/product.asp?bigtype="&root_id&"") then   
...
Response.Expires=0   '使缓存的页面立即过期      '清空存储在缓存中的页面
Response.Clear
